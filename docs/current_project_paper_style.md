...
截至 **2026 年 3 月 22 日**，仓库内已经有多组可直接追溯的 masked eval 结果。和早期原型阶段相比，当前结果已经明显收敛，尤其是 `exp30` 在训练中期取得了目前最好的 planning 表现。

下表统一汇总当前最关键的几组结果：

| 实验 | 结果来源 | Detection mAP | Detection NDS | Tracking AMOTA | Mapping mAP_normal | Motion EPA (car / pedestrian) | Planning L2 | Planning obj_box_col |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `exp27` | `work_dirs/sparsedrive_small_stage2_exp27/e2e_metrics.json` | 0.4015 | 0.5157 | 0.3656 | 0.5401 | 0.4869 / 0.4042 | 0.6268 | 0.0011 |
| `exp28` | `work_dirs/sparsedrive_small_stage2_exp28/e2e_metrics.json` | 0.3883 | 0.5042 | 0.3511 | 0.5124 | 0.4780 / 0.3952 | 0.6394 | 0.0011 |
| `exp30-final` | `work_dirs/sparsedrive_small_stage2_exp30/e2e_metrics.json` | 0.3936 | 0.5130 | 0.3541 | 0.5192 | 0.4785 / 0.3886 | 0.6569 | 0.0014 |
| `exp30-best@70325` | `work_dirs/sparsedrive_small_stage2_exp30/20260320_164041.log` 中第 1 次验证，对应 `iter_70325.pth` | 0.3988 | 0.5119 | 0.3515 | 0.5181 | 0.4793 / 0.3971 | **0.6232** | 0.0011 |

从这组表可以直接看到：

1. `exp30` 的**最佳 checkpoint 不是最后一次评估结果**，而是训练中期的 `iter_70325.pth`。
2. 和 `exp28` 相比，`exp30-best@70325` 在 detection、mapping、motion 和 planning 上都有一致提升，说明论文最小版结构是有效的。
3. 和当前最强的 `exp27` 相比，`exp30-best@70325` 的 detection / tracking 仍略低，但 planning `L2=0.6232` 已经略优于 `exp27` 的 `0.6268`。

因此，当前最适合作为论文主方法候选结果的，不是 `exp30-final`，而是 **`exp30-best@70325`**。
...
1. **系统已经跑通**  
   相机缺失模拟、补全链路、联合训练与端到端评测都已经接入完成，不再是纯设计阶段。`exp27`、`exp28`、`exp30` 三组结果已经形成了可追溯的实验序列。

2. **论文最小版结构已经表现出真实增益**  
   `exp30-best@70325` 相比 `exp28` 全面回升，说明将方法收缩为 `temporal completion + pv reconstruction` 这条主线后，结果比更复杂的堆叠式版本更稳，也更适合作为论文方法。

3. **训练稳定性仍是主要瓶颈**  
   `exp30` 的最佳结果出现在第一个评估点，后续继续训练反而使 planning `L2` 从 `0.6232` 退化到 `0.6569`。这说明当前主要问题已经不是“方法完全无效”，而是“最佳训练时机和训练日程还没有完全收敛”。

因此，更准确的表述不是“当前项目已经显著提升了最终指标”，而是：

> 当前项目已经完成了面向相机失效鲁棒性的核心方法实现，并在 `exp30-best@70325` 上取得了具有论文潜力的阶段性结果；下一步工作的重点不再是继续堆叠模块，而是围绕最佳 checkpoint 选择、训练日程和标准/缺失双评测继续收敛实验结论。
