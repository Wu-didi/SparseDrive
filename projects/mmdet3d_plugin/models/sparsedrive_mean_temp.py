...
class PVReconMeanTemporal(nn.Module):
    ...
    def _mean_fill(self, F, cam_mask):
        # 全部是张量运算，不要 no_grad，这样 loss 的梯度能流回 F（即 backbone/neck）
        B, V, C, H, W = F.shape
        ...
        mean_vis  = vis_sum / vis_count                   # [B,1,C,H,W]
        F_rec = F * vis + mean_vis * miss                 # 只在缺失视角写回
        return F_rec
...
class SparseDrive(BaseDetector):
    def __init__(
        ...
        # 使用refine 看下效果
        self.pv_recon = PVReconMeanTemporal(
            use_refine=True,          # 先关闭细化头，省时；稳定后可 True
            ch_per_scale= [256,256,256,256],         # use_refine=True 时需要填写，比如 [256,256,256,256]
            hist_momentum=1.0
        )
    ...
    def extract_feat(self, img, return_depth=False, metas=None, cam_mask=None):
        ...
        if "metas" in signature(self.img_backbone.forward).parameters:
            ...
            # feature_maps[0]: [B*V, 256, 64, 176]
            # feature_maps[1]: [B*V, 512, 32, 88]
            # feature_maps[2]: [B*V, 1024,16, 44]
            # feature_maps[3]: [B*V, 2048, 8, 22]
        if self.img_neck is not None:
            feature_maps = list(self.img_neck(feature_maps))  # list

        # 关键 reshape：展回 [B, V, C, H, W]
        for i, feat in enumerate(feature_maps):
            feature_maps[i] = torch.reshape(feat, (bs, num_cams) + feat.shape[1:])
            # 例如：
            # feature_maps[i].shape -> [B, V, C, H, W]
        ...
        # 注意：补全要发生在 deformable_func 之前，保持输入契约
    ...
    def simple_test(self, img, **data):
        # 常规推理默认不做遮挡；如需鲁棒评测，可在此也调用 cam_dropout(..., return_mask=True)
        feature_maps = self.extract_feat(img, metas=data.get('img_metas', None), cam_mask=None)
        model_outs = self.head(feature_maps, data)
