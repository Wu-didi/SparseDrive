...
class SparseDrive(BaseDetector):
    ...
    def extract_feat(self, img, return_depth=False, metas=None, cam_mask=None):
        ...
        # ====== 在这里做“VAE 视角级特征补全” ======
        if self.training:
            # 训练阶段：无论是否有缺失，都跑一次 VAE，避免 DDP unused parameters
            if cam_mask is None:
            ...
            # 测试阶段：仅在 cam_mask 有 True 时才做补全；否则用原始特征
            self.vae_loss_dict = None
    ...
    def simple_test(self, img, **data):
        ...
        feature_maps = self.extract_feat(
            img, metas=data.get('img_metas', None), cam_mask=cam_mask
        )
