...
class InstanceBank(nn.Module):
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        instance_feature = torch.tile( # 按给定次数在各维上把整个张量“平铺”复制
            self.instance_feature[None], (batch_size, 1, 1)
        )
        anchor = torch.tile(self.anchor[None], (batch_size, 1, 1))

        if (
            self.cached_anchor is not None # 当cached_anchor 不为none,或者batch size没发生改变的时候会重新计算下面的
            ...
            time_interval = instance_feature.new_tensor(
                [self.default_time_interval] * batch_size # deault_time_interval默认为0.5
            )
    ...
    def cache(
        ...
        ) = topk(confidence, self.num_temp_instances, instance_feature, anchor)  # 从当前前向结果中选出“临时跟踪的 Top-K 实例”并存入缓存（特征与锚点），同时用上一帧的分数做衰减/平滑，为下一帧的跨帧融合与 ID 继承做准备。逐句解析如下：
